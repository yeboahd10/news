      ...
      {CATEGORIES.map(cat => {
        ...
        return (
          <div key={cat} className="mb-6">
            <h2 className="w-full text-xl font-semibold bg-indigo-800 text-white p-2 rounded-md">{cat}</h2>
            ...
            <div className="mt-2 flex justify-center">
              {items.length > shown.length ? (
                <button onClick={() => navigate(`/category/${encodeURIComponent(cat)}`)} className="w-1/2 text-center text-lg font-semibold border border-indigo-800 text-indigo-800 p-1 rounded-md hover:bg-indigo-700">More</button>
              ) : (
